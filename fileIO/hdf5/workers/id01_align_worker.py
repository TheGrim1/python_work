...
from fileIO.edf.open_edf import read_multiframe_edf as open_edf

def id01_align_data_employer(pickledargs_fname):
    '''
...
def id01_align_data_worker(pickledargs_fname):
    '''
    ...
    angles = [float(x) for x in unpickled_args[2]]
    map_shape = [int(x) for x in unpickled_args[3]]
    shift = unpickled_args[4]
    verbose = unpickled_args[5]
    ...
    with h5py.File(target_fname) as target_file:        
        ...
        data = open_edf(source_fname)
        # cast to uint64 to avoid rounding issues and improve compression
        data = np.asarray(data*1000, dtype = np.uint64)
        datashape = data.shape
        data = data.reshape(list(map_shape)+list(datashape[1:]))

        data = data_stack_shift(data, -1*shift, None,order='inverted')
        ...
        target_group.create_dataset('eta', data=angles[0])
        target_group.create_dataset('phi', data=angles[1])
        target_group.create_dataset('del', data=angles[2])
...
if __name__=='__main__':
    ...
    id01_align_data_worker(pickledargs_fname)
