...
def reshape_worker(args):
    ...
    y = np.linspace(0.0,0.001*500*11+0.560,0.001)
    z = np.linspace(0.0,0.4,0.004)
    troi = ((1100, 480), (15, 130))
    
    sourcemaster_fname = parse_master_fname(data_fname)
    eigerrunno = get_eigerrunno(sourcemaster_fname)
    scan_col = int((eigerrunno-54))

      
    y_offset = scan_col*0.5
    yi_offset = scan_col*500

    yi,zi = np.meshgrid(range(yi_offset+560,yi_offset,-1),range(0,101,1))
    li,ki = np.meshgrid(range(560),range(101))
    indexes = zip(ki.flatten(),li.flatten(),zi.flatten(),yi.flatten())

    bigmeshshape = (101, 500*11+561)
...
def main(args):
    ...
    save_dir = '/hz/data/id13/inhouse10/THEDATA_I10_1/d_2018-04-29_inh_ihmi1397_aj/PROCESS/aj_log/analysis/scan2_reordered/'
