...
def do_regrouping(merged_fname, Q_dim, map_shape, par_dict, interp_factor, prefix='', verbose = True):
    ...
    with h5py.File(Qmerged_fname) as dest_h5:
        ...
        qx_ds = q_group.create_dataset(name='qx', shape=map_shape, dtype=np.float32, compression='lzf')
        qy_ds = q_group.create_dataset(name='qy', shape=map_shape, dtype=np.float32, compression='lzf')
        qz_ds = q_group.create_dataset(name='qz', shape=map_shape, dtype=np.float32, compression='lzf')
        q_ds = q_group.create_dataset(name='q', shape=map_shape, dtype=np.float32, compression='lzf')

        sx_ds = s_group.create_dataset(name='sx', shape=map_shape, dtype=np.float32, compression='lzf')
        sy_ds = s_group.create_dataset(name='sy', shape=map_shape, dtype=np.float32, compression='lzf')
        sz_ds = s_group.create_dataset(name='sz', shape=map_shape, dtype=np.float32, compression='lzf')
        s_ds = s_group.create_dataset(name='s', shape=map_shape, dtype=np.float32, compression='lzf')

        theta_ds = q_group.create_dataset(name='theta', shape=map_shape, dtype=np.float32, compression='lzf')
        phi_ds = q_group.create_dataset(name='phi', shape=map_shape, dtype=np.float32, compression='lzf')                
        ...
        for fname in result_fname_list:
            ...
            with h5py.File(fname,'r') as source_h5:
                ...
                # angles
                theta_ds[i,j] = np.arccos(qz/q)
                phi_ds[i,j] = np.arctan(qx/qy)
...
def main():
    ...
    prefix = 'q100_bin1_int1_'
    
    Q_dim = [nQx, nQy, nQz] = [100,100,100]
    map_shape = [100,100]
