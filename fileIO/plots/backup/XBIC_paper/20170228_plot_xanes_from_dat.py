...
xbic, xbicheader    = open_data(specpath + 'redo_xbic.dat', delimiter = '\t')
xanes200, xanes200header = open_data(specpath + 'redo_ga.dat', delimiter = '\t')
...
# GaAs
ax1 = plt.gca()
ax1.plot(gema[:,0], gema[:,4], linewidth = 2)
ax1.plot(gaasmeas_nocorr[:,0], gaasmeas_nocorr[:,1], linewidth = 2)
ax1.plot(gaasmeas_norm[:,0], gaasmeas_norm[:,1], linewidth = 2)
ax1.set_ylim(-0.1,1.8)
ax1.set_xlim(10360,10410)
ax1.set_xticklabels(['{:d}'.format(int(x)) for x in ax1.get_xticks()])
ax1.legend(['Marinez-Criado et al.','measured on GaAs wafer','... aligned'])
plt.title('GaAs XANES')
plt.show()
...
# xanesavg = np.zeros(shape = (len(xanes200[:,0]),2))
# xbicavg = np.zeros(shape = (len(xbic[:,0]),2))

# xanesavg[:,0] = xanes200[:,0]
# xbicavg[:,0] = xbic[:,0]

# xanesavg[:,1] = np.sum(xanes200[:,1::], axis = -1) /len(xanes200[0,1::])
# xbicavg[:,1] = np.sum(xbic[:,1::], axis = -1)      /len(xbic[0,1::])


# ax1.plot(xanesavg[:,0], xanesavg[:,1], color = 'black', linewidth = 2)
# ax1.plot(xbicavg[:,0], xbicavg[:,1], color = 'r', linewidth = 2)

# ax1.set_xlim((10360,10400))
# ax1.set_ylim((0,1.7))
