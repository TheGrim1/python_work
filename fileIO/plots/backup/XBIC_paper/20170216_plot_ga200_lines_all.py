...
def main():
    ...
    data1       = np.zeros(shape = (len(ga200[1,1,:]), np.sum(xbicmask) + 1))
    data1[:,0]  = energy200
...
#    ax1.set_xticks([x/1000.0 for x in xticks])
    ...
    ax1.set_ylabel('standardized Ga-K XRF signal')

    fig.tight_layout()
    savename = '/tmp_14_days/johannes1/lincom/spectra/redo_ga_nonstan'
