...
def main():
    ...
    data1       = np.zeros(shape = (len(xbic200[1,1,:]), np.sum(xbicmask) + 1))
    data1[:,0]  = energy200
...
#    ax1.set_xticks([x/1000.0 for x in xticks])
    ...
    ax1.set_ylabel('standardized XBIC signal')

    fig.tight_layout()
    savename = '/tmp_14_days/johannes1/lincom/spectra/redo_xbic'
    ...
    extent = 0 , 18, 34, 0
    im1 = plt.matshow(np.sum(xbic200_norm[:,:,:],axis = -1), cmap = plt.cm.gnuplot2, fignum = 0, extent = extent)
