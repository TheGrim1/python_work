...
xbic_stan, xbic_stanheader = open_data(specpath + 'redo_xbic.dat', delimiter = '\t')
...
xbic_stan[:,0]  += measured_correction
gaasmeas_norm[:,0] += measured_correction

xbic_stan   = crop_energyrange(xbic_stan,energyrange)
...
xbic_stanavg = np.zeros(shape = (len(xbic_stan[:,0]),2))


xanesavg[:,0] = xanes[:,0]
xbicavg[:,0] = xbic[:,0]
xbic_stanavg[:,0] = xbic_stan[:,0]


xanesavg[:,1] = old_div(np.sum(xanes[:,1::], axis = -1),len(xanes[0,1::]))
xbicavg[:,1] = old_div(np.sum(xbic[:,1::], axis = -1),len(xbic[0,1::]))
xbic_stanavg[:,1] = old_div(np.sum(xbic_stan[:,1::], axis = -1),len(xbic_stan[0,1::]))
print(xbicavg)
...
ax1.plot(xbic_stanavg[:,0], xbic_stanavg[:,1], color = 'black', linewidth = 2)


ax1.set_xlim((10355,10400))
ax1.set_ylim((0,1.7))
ax1.legend(['Ga-K XRF','XBIC, measured','XBIC, standardized'])
...
ax2 = ax1.twinx()
ax2.set_xlim((10360,10400))
ax2.set_ylim((0,85))
ax2.plot(xbicavg[:,0], xbicavg[:,1], color = 'blue', linewidth = 2)
ax2.set_ylabel('measured current [pA]', color='blue')
