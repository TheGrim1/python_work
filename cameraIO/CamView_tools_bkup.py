...
##### handling motors
    ...
    def _correct_with_lookup(self, function, position):
        print 'correcting movement of %s with motors:' % function
        if function in self.lookup.keys():
            for mot in self.lookup[function].values():
                if mot != function:
                    correction = np.interp(position, self.lookup[function][function], self.lookup[function][mot])
    ...
    def mvr(self, function, distance,
        ...
        if move_using_lookup:
            position = self.wm(function)
            self._correct_with_lookup(function, position)
...
##### centering functionality:
    ...
    def _get_imagestack(self, view, motor, view, plot, positions, troi = None, midcontrast = 0.5, backlashcorrection = True):
    ...
    def get_COR(self, 
        ...
        if plot and not (mode.upper() == 'COM' and view == 'side'):
            ...
            self._show_CORresults(imagestack, aligned, thetas)
...
##### lookuptable functionality
    def _save_lookup(self, function, savename):
        ...
        header = self.lookup[function].keys()
    ...
    def make_lookup(self,
        ...
        print 'will try to get a lookuptable to align rotation in %s with motors %s an %s' %(motor, mot0, mot1)
        ...
        self.lookup[motor][:,0]  = thetas
        self.lookup[motor][:,1]  = shift[:][0]*self.calibration[mot0]
        self.lookup[motor][:,2]  = shift[:][1]*self.calibration[mot1]

        if type(savename) != type(None):
            self._save_lookup(savename,self.lookup[motor],header=[motor, mot0, mot1])
                        
    return 1
    
##### some cosmetic plotting
    def _show_CORresults(self, imagestack, aligned, thetas):
